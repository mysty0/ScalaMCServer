...
object InventoryItem{
  def apply(slotRaw: SlotRaw): InventoryItem =
    Items.getClass.getDeclaredFields map {field => field.get(Items).asInstanceOf[InventoryItem]} find {item => item.id==slotRaw.itemId && item.metadata == slotRaw.itemDamage.getOrElse(0)} getOrElse new InventoryItem(0)
